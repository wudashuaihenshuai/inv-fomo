...
def get_args_parser():
    ...
    # parser.add_argument('--test_set', default='test_gt.txt', help='testing txt files')
    ...
    parser.add_argument('--image_inv_file', default='few_shot_data_cit.json', type=str)
    ...
    # parser.add_argument('--post_process_method', default='attributes',
    #                 help='seperated: Used for the fs baseline attributes: Used for attribute experiments')
    ...
    parser.add_argument('--use_prior_pro', action='store_true')
    parser.add_argument('--tune_head', action='store_true')
    parser.add_argument('--adaptor', action='store_true')
    parser.add_argument('--tune_att_W', action='store_true')
    parser.add_argument('--k_attributes', default=1, type=int)
    parser.add_argument('--train_prior_loss', default='./loss/prior_prob_training_losses_.txt', type=str)
    parser.add_argument('--attribute_file', default='', type=str)
    parser.add_argument('--build_known_unknown_file', default='classnames.txt', type=str)
    parser.add_argument('--image_embeds', default='all_embeds.pt', type=str)
    return parser
...
def main(args):
    ...
    best_kmap  = -1 
    bad        = 0
    ####如果性能持续提升（K_AP50 一直变好）	全部 9 次
    ####如果性能停滞（连续 3 次未提升）	仅执行前 3 次
    if (len(neg_sup_ep) > 1 or len(neg_sup_lr) > 1) and args.image_conditioned and args.att_refinement:
        for eps in tqdm(neg_sup_ep, desc='Epochs', leave=False):
            for lr in tqdm(neg_sup_lr, desc='lr', leave=False):
                ...
                # model, postprocessors = build_templates_filemodel(args)
                model, postprocessors = build_model2(args)
                model.to(device)
